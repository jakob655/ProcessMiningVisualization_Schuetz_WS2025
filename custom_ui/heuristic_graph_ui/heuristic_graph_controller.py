...
class HeuristicGraphController:
    def __init__(self, workingDirectory, dependency_threshold=0.5, spm_threshold=0.0, min_frequency=1):
        super().__init__()
        self.model = None
        self.workingDirectory = workingDirectory
        self.default_dependency_threshold = dependency_threshold
        self.default_spm_threshold = spm_threshold
    ...
    def startMining(self, cases):
        ...
        self.create_dependency_graph(self.default_dependency_threshold, self.default_spm_threshold,
                                     self.default_min_frequency)

    # CALL BEFORE USAGE (option 2 for mining existing models)
    def loadModel(self, file_path):
        self.model = pickle_load(file_path)
        self.create_dependency_graph(self.get_threshold(), self.get_spm_threshold(), self.get_min_frequency())
        return file_path

    def create_dependency_graph(self, dependency_threshold, spm_threshold, min_frequency):
        graph = self.model.create_dependency_graph_with_graphviz(dependency_threshold, spm_threshold, min_frequency)
        graph.render(self.workingDirectory, format='dot')
        # print("HeuristicGraphController: CSV mined")
        return graph

    def get_spm_threshold(self):
        return self.model.get_spm_threshold()
